...
stray_chars = ['..', '==', '++', '--', '//', '``'] #List of stray characters which must be removed
stemmer = PorterStemmer()

path = "/home/aditya/Data Science/IR Project/Documents_Mega/"
...
"""Till this point it was pre-processing. The docs were opened, read, tokenized, stemmed, and a posting list was created. Now, we handle queries from user"""
while(True):
    ...
    query = input("What can I search for you?\n").lower() #Input from the user and turn it into lowercase
    if(query == "quit()"):
        break
    ...
    for index, word in enumerate(query_stem):
        ...
        #WILDCARD QUERIES    
            ...
            match_list.append(final_wildcard_list)
    ...
    print(final_list)
    print("RANKED FINAL LIST")
    print(sorted(tf_idf, key = tf_idf.get, reverse = True)) #Print the sorted final list (Based on rank)
       
for i in range(1, file_count+1):    #Close all files
    fileopen[i].close() 
