//
//  Candidate Elimination for Concept Learning
//
//
//  
//
...
int main()
{
  ...
  */
  ...
  cout << "\nversion space\n";
  set<vector<int> > vs;
  queue<vector<int> > tempvs;
  vector<int> tempg, temps;
  temps = *(specific.begin());
  ...
  for(set< vector<int> >::iterator it= vs.begin(); it!=vs.end(); it++)
  {
    abc = *it;
    cout<<"< ";
    for(int i=0; i< numberFeatures; i++)
